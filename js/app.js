...
function renderSidebarWidgets() {
  ...
  // 3. Nube de Etiquetas
  const tagsBox = document.getElementById('sidebar-tags');
  if (tagsBox && typeof BLOG_TAGS !== 'undefined') {
    tagsBox.innerHTML = BLOG_TAGS.map(tag => `
      <span class="tag-cloud-item" onclick="filterByTag('${tag.replace('#', '')}')">#${tag.replace('#', '')}</span>
    `).join('');
  }
}

/**
 * Renderiza la Cuadrícula del Equipo Editorial & Investigadores
 */
  ...
  grid.innerHTML = TEAM_MEMBERS.map(member => `
    <article class="author-profile-card">
      ...
      <div class="author-profile-column">
        Columna: ${member.role.split('&')[0].trim()}
      </div>
    </article>
  `).join('');
}
...
function renderWorks() {
  ...
  container.innerHTML = filtered.map(work => {
    ...
    if (work.status === 'completed') {
      statusBadge = `<span class="post-card-category-badge" style="color: #34d399; border-color: rgba(16,185,129,0.3);">Publicado</span>`;
    } else if (work.status === 'in_progress') {
      statusBadge = `<span class="post-card-category-badge" style="color: #38bdf8; border-color: rgba(6,182,212,0.3);">En Redacción</span>`;
    } else {
      statusBadge = `<span class="post-card-category-badge" style="color: #fbbf24; border-color: rgba(245,158,11,0.3);">Próximo Ensayo</span>`;
    }
    ...
    ` : `
      <div class="post-card-thumb-placeholder">
        ...
        <span style="font-size: 0.76rem; font-family: var(--font-mono);">${work.code}</span>
      </div>
    `;
        ...
        <div class="post-card-body">
          <div class="post-card-header">
            <div class="post-card-meta">
              ...
              <span>•</span>
              <span>⏱️ ${work.readTime || '5 min de lectura'}</span>
              <span>•</span>
              <span style="color: var(--accent-cyan); font-weight: 600;">${work.category || 'Filosofía Práctica'}</span>
            </div>
...
function openArticleReader(workId) {
  ...
  document.getElementById('reader-read-time').textContent = work.readTime || '5 min de lectura';
  ...
  mainContent.innerHTML = `
    <p style="font-size: 1.12rem; line-height: 1.85; color: #f1f5f9;">
      ${work.summary}
    </p>
    ${work.details && work.details.teamPurpose ? `
      <div style="background: rgba(9, 13, 22, 0.7); padding: 22px; border-radius: var(--radius-md); border-left: 3px solid var(--accent-cyan); margin: 18px 0;">
        <h4 style="font-family: var(--font-heading); color: var(--accent-cyan); font-size: 1rem; margin-bottom: 8px;">Compromiso & Reflexión del Colectivo:</h4>
        <p style="font-size: 0.95rem; color: var(--text-main); line-height: 1.7;">${work.details.teamPurpose}</p>
    ...
    ${work.details && work.details.commitment ? `
      <div style="background: rgba(9, 13, 22, 0.7); padding: 22px; border-radius: var(--radius-md); border-left: 3px solid var(--accent-emerald); margin: 18px 0;">
        ...
        <p style="font-size: 0.95rem; color: var(--text-main); line-height: 1.7;">${work.details.commitment}</p>
      </div>
    ` : ''}
  `;
  ...
  if (work.id === 'ac1') {
    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 18px;">
        <h4 style="font-family: var(--font-heading); font-size: 1.25rem; color: #ffffff;">Metáfora Artística: El telos del robot ROZZUM 7134</h4>
        ...
        <div style="background: rgba(9, 13, 22, 0.7); padding: 20px; border-radius: var(--radius-md); border-left: 3px solid var(--accent-cyan);">
          <h5 style="color: var(--accent-cyan); font-family: var(--font-heading); margin-bottom: 6px;">Analogía Deontológica:</h5>
    ...
    const popRows = (work.vulnerablePopulations || []).map(p => `
      <tr>
        ...
        <td style="color: var(--accent-cyan); font-weight: 600;">${p.principle}</td>
      </tr>
    `).join('');

    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 20px;">
        <h4 style="font-family: var(--font-heading); font-size: 1.25rem; color: #ffffff;">Mapa de Rostros Vulnerables ante la Técnica en el Perú</h4>
    ...
    const schelerRows = (work.schelerMatrix || []).map(row => `
      <tr>
        ...
        <td>
          <span style="padding: 3px 8px; border-radius: 4px; font-family: var(--font-mono); font-size: 0.76rem; font-weight: 700; background: ${row.presence === 'Sí' ? 'rgba(16,185,129,0.2)' : row.presence === 'Parcial' ? 'rgba(245,158,11,0.2)' : 'rgba(244,63,94,0.2)'}; color: ${row.presence === 'Sí' ? '#34d399' : row.presence === 'Parcial' ? '#fbbf24' : '#f87171'};">
            ${row.presence}
          </span>
        </td>
    ...
    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 20px;">
        <h4 style="font-family: var(--font-heading); font-size: 1.25rem; color: #ffffff;">Pirámide Jerárquica de Valores DeonTech</h4>
        <div style="border-radius: var(--radius-lg); overflow: hidden; max-height: 340px; border: 1px solid var(--border-subtle);">
          <img src="assets/img/ac3-jerarquia-valores.jpg" alt="Pirámide de Valores">
        </div>
        <div style="background: rgba(9, 13, 22, 0.7); padding: 18px; border-radius: var(--radius-md); border-left: 3px solid var(--accent-amber);">
        ...
        <h4 style="font-family: var(--font-heading); font-size: 1.2rem; color: #ffffff;">Tabla Axiológica bajo la Escala de Max Scheler</h4>
        <div class="table-responsive">
          <table class="custom-table">
            <thead>
              <tr>
    ...
    evidenceBox.innerHTML = `
      <div style="padding: 24px; background: rgba(9, 13, 22, 0.6); border-radius: var(--radius-md); border: 1px solid var(--border-subtle);">
...
function setupEventListeners() {
  ...
  const openAddBtn = document.getElementById('open-add-work-btn');
  if (openAddBtn) {
    openAddBtn.addEventListener('click', openAddWorkModal);
  }
  ...
  const addModal = document.getElementById('add-work-modal');
  if (addModal) {
    addModal.addEventListener('click', (e) => {
      if (e.target === addModal) closeAddWorkModal();
    });
  }
}
...
/**
 * Gestión del Formulario de Nueva Publicación
 */
...
function handleCreateWork(event) {
  ...
  const readTime = document.getElementById('form-readtime').value.trim() || '5 min de lectura';
  ...
  const imgUrl = document.getElementById('form-img').value.trim();
  const pdfUrl = document.getElementById('form-pdf').value.trim();

  const tags = tagsRaw ? tagsRaw.split(',').map(t => t.trim()).filter(Boolean) : ['Ética', 'Deontología'];
  ...
  const newPost = {
    ...
    readTime: readTime,
    author: 'Equipo Editorial DeonTech',
    status: status,
    featured: false,
    thumbnail: imgUrl,
    pdfUrl: pdfUrl,
    ...
    theories: [
      {
        concept: "Deontología y Responsabilidad Profesional",
        source: "Marco Editorial DeonTech",
        application: "Articulación entre conocimientos técnicos y el compromiso ciudadano con los derechos humanos."
      }
    ],
    details: {
      teamPurpose: purpose
  ...
  allTrabajos.unshift(newPost);
  saveTrabajos(allTrabajos);

  renderWorks();
  closeAddWorkModal();
  showToast(`¡Artículo "${newPost.title.slice(0, 25)}..." publicado en el blog!`, 'success');
}

function handleResetData() {
  if (confirm('¿Deseas restablecer los artículos del blog a las publicaciones originales (AC1, AC2 y AC3)?')) {
    allTrabajos = resetTrabajos();
    renderWorks();
    showToast('Blog restablecido a sus publicaciones originales.', 'info');
  }
}
...
function handleNewsletterSubmit(event) {
  ...
  if (input) {
    showToast(`¡Gracias! Te hemos suscrito al boletín de DeonTech (${input.value}).`, 'success');
    input.value = '';
  }
}
