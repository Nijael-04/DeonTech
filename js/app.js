...
function renderWorks() {
  ...
  container.innerHTML = filtered.map(work => {
    ...
    const apaTag = work.id === 'ac1'
      ? 'Nota. The Wild Robot (Sanders, 2024)'
      : 'Nota. Elaboración propia';

    const mediaHtml = work.thumbnail ? `
      <img src="${work.thumbnail}" alt="${work.title}" loading="lazy">
      <span class="post-card-apa-tag">${apaTag}</span>
    ` : `
...
function openArticleReader(workId) {
  ...
  if (work.thumbnail) {
    ...
    if (work.id === 'ac1') {
      captionEl.innerHTML = '<strong>Nota.</strong> De la película <em>The Wild Robot</em> (Sanders, 2024), DreamWorks Animation.';
    } else {
      captionEl.innerHTML = '<strong>Nota.</strong> Elaboración propia.';
    }
  } else {
  ...
  if (work.id === 'ac1') {
    ...
    pullQuote.textContent = "«Ser honesto intelectualmente implica reconocer las fallas del propio campo profesional, no solo las del ‘otro’. El mapa de negligencias es un ejercicio de honestidad intelectual colectiva.»";
    pullCite.textContent = "— Diagnóstico Crítico del Manifiesto DeonTech (AC4)";
  } else {
  ...
  if (work.id === 'ac1') {
    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 18px;">
        ...
        <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: -8px; font-style: italic;"><strong>Nota.</strong> De la película <em>The Wild Robot</em> (Sanders, 2024), DreamWorks Animation.</p>
    ...
    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: -10px; font-style: italic;"><strong>Nota.</strong> Elaboración propia.</p>
    ...
    const negRows = (work.negligenciasMap || []).map(row => `
      <tr>
        <td style="color: var(--text-main); font-weight: 600;">${row.negligencia}</td>
        <td style="color: #dc2626; font-weight: 600;">${row.falacia}</td>
        <td style="color: var(--text-muted);">${row.poblacion}</td>
        <td style="color: #059669; font-weight: 600;">${row.accionPositiva}</td>
      </tr>
    `).join('');

    evidenceBox.innerHTML = `
      <div style="display: flex; flex-direction: column; gap: 24px;">
        <h4 style="font-family: var(--font-heading); font-size: 1.25rem; color: var(--text-main);">Infografía de la Actividad Colaborativa 4: Diagnóstico Ético</h4>
        <div style="border-radius: var(--radius-lg); overflow: hidden; border: 1px solid var(--border-subtle); background: #f8fafc; text-align: center;">
          <img src="assets/img/ac4-infografia-colaborativa.jpg" alt="Infografía AC4 - Mapa de Negligencias Éticas" style="max-height: 560px; width: auto; max-width: 100%; object-fit: contain; margin: 0 auto; display: block;">
        </div>
        <p style="font-size: 0.8rem; color: var(--text-muted); margin-top: -14px; font-style: italic;"><strong>Nota.</strong> Elaboración propia.</p>

        <h4 style="font-family: var(--font-heading); font-size: 1.25rem; color: var(--text-main);">Mapa de Negligencias Éticas y Acciones Positivas</h4>
        <div class="table-responsive">
          <table class="custom-table">
            <thead>
              <tr>
                <th>Negligencia o Falla Ética</th>
                <th>Falacia o Distorsión que la Sostiene</th>
                <th>Población Vulnerable Afectada (AC2)</th>
                <th>Acciones Positivas de la Profesión</th>
              </tr>
            </thead>
            <tbody>
              ${negRows}
        ...
        ${work.socraticQuestion ? `
        <div style="background: linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%); padding: 22px; border-radius: var(--radius-md); border: 1px solid var(--border-subtle); border-left: 4px solid var(--primary-blue);">
          <h5 style="color: var(--primary-blue); font-family: var(--font-heading); margin-bottom: 8px; font-size: 0.95rem;">🤔 Pregunta Socrática del Equipo:</h5>
          <p style="font-size: 1.05rem; color: var(--text-main); line-height: 1.7; font-style: italic; font-weight: 600;">«${work.socraticQuestion}»</p>
        </div>
        ` : ''}

        ${work.manifestoClosure ? `
        <div style="background: linear-gradient(135deg, #fef3c7 0%, #fff7ed 100%); padding: 24px; border-radius: var(--radius-md); border: 1px solid #fde68a; border-left: 4px solid var(--accent-orange);">
          <h5 style="color: var(--accent-orange); font-family: var(--font-heading); margin-bottom: 10px; font-size: 1rem;">📜 Cierre del Manifiesto de Identidad Ética (AC1 + AC2 + AC3 + AC4):</h5>
          <p style="font-size: 1rem; color: var(--text-main); line-height: 1.8; font-weight: 500;">${work.manifestoClosure}</p>
        </div>
        ` : ''}
      </div>
    `;
